...
export const rebuildAndSync =
  async (): Promise<RebuildResult> => {
    const startedAt =
      new Date();

    const job =
      await prisma.merkleJob.create({
        data: {
          distributionType:
            DistributionType.AIRDROP,

          status:
            MerkleJobStatus.PROCESSING,
...
export const cronRebuild =
  async (): Promise<RebuildResult> => {
    const needed =
      await isRebuildNeeded();
